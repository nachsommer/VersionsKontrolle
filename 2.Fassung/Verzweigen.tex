...
\begin{description}
\item[1.~Schritt:] Einlesen eines Beispieltexts, eines Text-Korpus, eines ganzen Werks zur Stilanalyse.
\item[2.~Schritt:] Textanalyse. Da sich diese Art der stilistischen Mimesis oder Originalkopie aus der \emph{copia verborum} speist, aus der Fülle der Wörter, gilt es die eingelesenen Buchstabenmengen zu gewichten, zu sortieren und zu überprüfen, und zwar mit einer einfachen Statistik auf die Übergangswahrscheinlichkeiten von einem Wort zum nächsten. Mit diesen Reihen von Worten zu Worten zu Worten, die sich ohne grösseren Aufwand computertechnisch mit Hilfe von Markov-Ketten modellieren lassen,\footcite[Zur Geschichte dieses Algorithmus vgl.][]{hilgers:2007} erhält jedes Wort in einem Text einen numerischen Wert, der angibt, mit welcher Wahrscheinlichkeit das eine Wort auf das andere folgt. 
